...
func fillTimeSeries(ts *TimeSeriesBinary, m *types.Metric) *TimeSeriesBinary {
	if cap(ts.LabelsNames) < len(m.Labels) {
		ts.LabelsNames = make([]ByteString, len(m.Labels))
	} else {
		ts.LabelsNames = ts.LabelsNames[:len(m.Labels)]
	}

	if cap(ts.LabelsValues) < len(m.Labels) {
		ts.LabelsValues = make([]ByteString, len(m.Labels))
	} else {
	...
	for i, v := range m.Labels {
		ts.LabelsNames[i] = unsafe.Slice(unsafe.StringData(v.Name), len(v.Name))
		ts.LabelsValues[i] = unsafe.Slice(unsafe.StringData(v.Value), len(v.Value))
	}
	return ts
}
	...
	for seriesIndex, series := range sg.Series {
		...
		for i := range series.LabelsNames {
			metric.Labels[i] = labels.Label{
				Name:  series.LabelsNames[i].String(),
				Value: series.LabelsValues[i].String(),
			}
		}
