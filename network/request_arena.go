...
func createWriteRequest(series []*types.TimeSeriesBinary, externalLabels map[string]string) ([]byte, error) {
	mem := arena.NewArena()
	defer mem.Free()
	sliceTS := arena.MakeSlice[prompb.TimeSeries](mem, len(series), len(series))
	wr := arena.New[prompb.WriteRequest](mem)
	wr.Timeseries = sliceTS

	for i, tsBuf := range series {
		ts := wr.Timeseries[i]
		ts.Labels = arena.MakeSlice[prompb.Label](mem, len(tsBuf.Labels), len(tsBuf.Labels))
